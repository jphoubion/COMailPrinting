...
class Printing:
    ...
    def create_pages(self, co, client, reference, company, company_type):
        now = datetime.now()
        address = co.split('\n')
        self.PDFFile.setFont("Helvetica", 10)
        self.PDFFile.drawRightString(19 * cm, 27 * cm, f'Anhée, le {now.strftime("%d/%m/%Y")}')
        self.PDFFile.drawString(12 * cm,26 * cm, f"MAITRE {address[0]}")
        self.PDFFile.drawString(12 * cm, 25.5 * cm, address[1])
        self.PDFFile.drawString(12 * cm, 25 * cm, address[2])
        self.PDFFile.drawString(13.5 * cm, 25 * cm, address[3])
